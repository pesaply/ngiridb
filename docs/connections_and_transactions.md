...
Ngiridb handles database connection pooling and transaction management
...
```python
async def add_excitement(connnection: ngiridb.core.Connection, id: int):
...
async with Database(database_url) as database:
    async with database.transaction():
        ...
        await database.execute(
            "INSERT INTO notes(id, text) values (1, 'ngiridb is cool')"
        )
